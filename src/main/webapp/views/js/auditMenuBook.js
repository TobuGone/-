...
//审核通过
var gridAdopt = function(rowIndex){
	$('#adopt-data-form').form('clear');// 清除form表单数据
	
	$( "#adopt-dialog" ).dialog("open").dialog('setTitle','审核通过');
	
	mygrid.datagrid('selectRow', rowIndex); 
	
	var rowData = mygrid.datagrid('getSelected');// 取到选择的数据
	
	$("#adoptId").val(rowData.id);
...
var saveAdopt = function(){
	var url = dataOptions.baseUrl+'saveAdopt.htm';/* 配置保存数据地址 */
	...
	var success = function(ret){
		if(ret && ret.status == 10001){
			$('#adopt-dialog').dialog('close');
			
			mygrid.datagrid('reload');
			
			$('#adopt-data-form').form('clear');
	...
	var error = null; // 请求服务器失败回调函数error 传递null使用默认error处理
	formSubmit($('#adopt-data-form'),url,check,success);
};


//审核不通过
var gridNoAdopt = function(rowIndex){
	$('#noadopt-data-form').form('clear');// 清除form表单数据
	
	$( "#noadopt-dialog" ).dialog("open").dialog('setTitle','审核不通过');
	
	mygrid.datagrid('selectRow', rowIndex); 
	
	var rowData = mygrid.datagrid('getSelected');// 取到选择的数据
	
	$("#noadoptId").val(rowData.id);
};

var saveNoAdopt = function(){
	var url = dataOptions.baseUrl+'saveNoAdopt.htm';/* 配置保存数据地址 */
	...
	var success = function(ret){
		if(ret && ret.status == 10001){
			$('#noadopt-dialog').dialog('close');
			
			mygrid.datagrid('reload');
			
			$('#noadopt-data-form').form('clear');
	...
	var error = null; // 请求服务器失败回调函数error 传递null使用默认error处理
	formSubmit($('#noadopt-data-form'),url,check,success);
};
