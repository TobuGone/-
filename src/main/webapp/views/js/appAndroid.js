...
var saveUser = function(){
	...
	var fileName = name.substring(name.lastIndexOf(".")+1,name.length);
	  	if( fileName !="apk" && fileName !="apk"){
	  		$("#spMsg").text("文件格式错误,请选择apk后缀名文件");
	        return;
	    }else{
		 	$("#spMsg").text('');
	    }
	  	var url = dataOptions.baseUrl+'saveAppAndroid.htm';/* 配置保存数据地址 */
...
var gridDel = function(rowIndex){
	...
	var delUrl = dataOptions.baseUrl + 'delAppAndroid.htm';/* 配置删除的地址 */
